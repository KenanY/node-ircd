...
Channel.prototype.send = function(message) {
	for(var u in this.users) {
		if(!Util.cmp(u, message.sender.nick) || message.command != 'PRIVMSG') {
...
Channel.prototype.join = function(user) {
	...
	user.send(Message.createMessage(user.server, Constants.RPL_ENDOFNAMES, nick, this.name, "End of NAMES list"))
	return true
}
...
// -----------------------------
// sets the topic of the channel, according to the user setting it.
//
